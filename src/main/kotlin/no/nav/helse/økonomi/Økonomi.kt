...
data class Økonomi(
        ...
        private fun totalSykdomsgrad(
            ...
            val totalgrad =
                Inntekt.vektlagtGjennomsnitt(
                    økonomiList.map { gradStrategi(it) to it.aktuellDagsinntekt },
                    økonomiList.inntektjustering(),
                )
            return totalgrad
        }
    ...
    init {
        require(dekningsgrad == DekningsgradSelvstendig || dekningsgrad == DekningsgradArbeidstaker) {
            "dekningsgrad må være 100 % eller 80 % var $dekningsgrad."
        }
    }
