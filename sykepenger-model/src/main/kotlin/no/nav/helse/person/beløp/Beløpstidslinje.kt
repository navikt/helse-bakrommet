...
data class Beløpstidslinje(private val dager: SortedMap<LocalDate, Beløpsdag>) : Collection<Dag> by dager.values {
    ...
    operator fun plus(other: Beløpstidslinje) =
        merge(other) { champion, challenger ->
            if (challenger.kilde.tidsstempel >= champion.kilde.tidsstempel) {
                challenger
            } else {
                champion
            }
    ...
    private fun merge(
        ...
        strategy: (champion: Beløpsdag, challenger: Beløpsdag) -> Beløpsdag,
    ): Beløpstidslinje {
    ...
    internal operator fun minus(other: Beløpstidslinje) =
        Beløpstidslinje(
            this.dager.filterValues { it.beløp != other.dager[it.dato]?.beløp },
        )
    ...
    internal fun kunIngenRefusjon() =
        filterIsInstance<Beløpsdag>().let { beløpsdager ->
            if (beløpsdager.isEmpty()) {
                false
            } else {
                beløpsdager.all { it.beløp == INGEN }
            }
        }
