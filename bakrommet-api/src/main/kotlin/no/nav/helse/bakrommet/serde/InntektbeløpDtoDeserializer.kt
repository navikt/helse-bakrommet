...
/**
 * Custom deserializer for InntektbeløpDto.DagligInt som håndterer både int og double verdier.
 * Dette løser problemet når frontend sender hele tall uten desimaler.
 */
class InntektbeløpDtoDagligIntDeserializer : JsonDeserializer<InntektbeløpDto.DagligInt>() {
    override fun deserialize(
        p: JsonParser,
        ctxt: DeserializationContext,
    ): InntektbeløpDto.DagligInt =
        when (p.currentToken) {
            JsonToken.VALUE_NUMBER_INT -> InntektbeløpDto.DagligInt(p.intValue)
            JsonToken.VALUE_NUMBER_FLOAT -> InntektbeløpDto.DagligInt(p.doubleValue.toInt())
            JsonToken.START_OBJECT -> {
                // Håndter objekt med beløp-felt: {"beløp": 200}
                var beløp: Int? = null
                p.nextToken() // Gå til første felt
                while (p.currentToken != JsonToken.END_OBJECT) {
                    if (p.currentName == "beløp") {
                ...
                if (beløp == null) {
                    throw JsonMappingException(p, "Missing beløp field in InntektbeløpDto.DagligInt")
                }
                InntektbeløpDto.DagligInt(beløp)
            }
            else -> throw JsonMappingException(p, "Expected number or object with beløp field")
        }
}
